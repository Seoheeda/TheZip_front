...
const InterestAreaSearch = () => {
  ...
  const handleAreaChange = (area) => {
    ...
    setDropdownView(false);
    navigate(`/realprice_map/dong/${area.dongCode}`);
  };

  return (
    <div
      className="flex justify-between items-center px-4 py-5 w-96 rounded-sm bg-white z-10"
      ref={dropdownRef}
    >
      {/* 관심지역 선택 버튼 */}
      ...
      <div className="relative">
        ...
        {dropdownView && (
          <ul className="absolute w-full bg-white max-h-56 overflow-auto custom-scrollbar border border-gray-2 mt-2 rounded-lg text-sm space-y-1 z-20">
