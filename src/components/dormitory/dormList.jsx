...
const DormList = ({ name, collegeId, setDormId, collegeAddress }) => {
  ...
  const onFilterToggle = () => {
    if (isFiltered) {
      ...
      // "대학생" 필터링
      const filteredList = originalDormList.filter((dorm) => dorm.dormitoryKind === "대학생");
  ...
  const onSortByLowPrice = () => {
    const sortedList = [...dormList].sort((a, b) => a.yearlyRent - b.yearlyRent);
    setDormList(sortedList);
  };

  const onSortByHighPrice = () => {
    const sortedList = [...dormList].sort((a, b) => b.yearlyRent - a.yearlyRent);
    setDormList(sortedList);
  };
    ...
    <div className="relative w-full max-h-[500px] h-full overflow-y-auto custom-scrollbar-big mx-auto bg-white border rounded-lg shadow-lg p-6">
      {dormList.length > 0 && (
        <ul className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <h2 className="col-span-full text-xl font-semibold mb-4 ml-2">기숙사 목록</h2>
          ...
          {dormList.map((dorm) => (
            ...
            >
              <h3 className="text-xl font-semibold text-gray-800 mb-4">{dorm.name}</h3>
              <table className="min-w-full table-auto text-lg">
                <tbody>
                  <tr className="border-b">
                    <td className="px-4 py-2 font-semibold text-gray-600">종류</td>
                    <td className="px-4 py-2 text-gray-800">{dorm.dormitoryKind}</td>
                  </tr>
                  <tr className="border-b">
                    <td className="px-4 py-2 font-semibold text-gray-600">방 번호</td>
                    <td className="px-4 py-2 text-gray-800">{dorm.roomNumber}</td>
                  </tr>
                  <tr className="border-b">
                    <td className="px-4 py-2 font-semibold text-gray-600">수용 인원</td>
                    <td className="px-4 py-2 text-gray-800">{dorm.capacity}명</td>
                  </tr>
                  <tr className="border-b">
                    <td className="px-4 py-2 font-semibold text-gray-600">월세</td>
                    <td className="px-4 py-2 text-gray-800">{dorm.rent.toLocaleString()}원</td>
                  </tr>
                  <tr className="border-b">
                  ...
                  <tr className="border-b">
                    ...
                    <td className="px-4 py-2 text-gray-800">{dorm.deposit.toLocaleString()}원</td>
                  </tr>
                  <tr>
      ...
      {collegeId === "" && (
        <p className="text-center text-gray-600 font-medium mt-8">
          <span className="block text-xl font-bold text-gray-800 mb-2">학교를 선택하세요!</span>
          선택된 학교가 없어요. 위에서 학교를 선택해 기숙사 정보를 확인해보세요. 🏫
        </p>
      )}
      {collegeId !== "" && dormList.length === 0 && (
        <p className="text-center text-gray-600 font-medium mt-8">
          <span className="block text-xl font-bold text-gray-800 mb-2">기숙사 정보가 없어요!</span>
