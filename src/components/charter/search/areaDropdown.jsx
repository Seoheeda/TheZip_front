  ...
  const getList = async () => {
    try {
      ...
      if (area.sido === null) {
        ...
        navigate(`/charters/dong/${response.data}`);
      }
    } catch (error) {
  ...
  return (
    <div className="flex w-full p-3 space-x-2 justify-center" ref={dropdownRef}>
      ...
      <div className="bg-primary-4 w-28 h-7 px-1 text-center rounded-xl items-center cursor-pointer">
        ...
        {dropdownView.district && (
          <ul className="bg-white max-h-64 overflow-auto custom-scrollbar border border-gray-2 mt-3 rounded-lg text-sm space-y-1 z-50 relative">
            {gugunList.length > 0 ? (
